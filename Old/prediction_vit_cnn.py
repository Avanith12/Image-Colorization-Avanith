...
class LossPlotCallback(LearningRateMonitor):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        # Store losses in CSV for tracking
        log_path = os.path.join(CACHE_DIR, LOSS_LOG_FILENAME)
        ...
        plt.legend()
        plt.grid(True)
        plot_path = os.path.join(CACHE_DIR, LOSS_PLOT_FILENAME)
...
# ====================== CALLBACK: LOSS TEXT LOGGING ====================== #
class LossLoggingCallback(LearningRateMonitor):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        train_loss = metrics.get("train_loss", None)
        val_loss = metrics.get("val_loss", None)
        self.text_logger.log_epoch(trainer.current_epoch + 1, train_loss, val_loss)
...
class ColorizationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_np = np.array(img.permute(1, 2, 0))  # (H, W, C)
        img_lab = rgb2lab(img_np).astype("float32")
        img_l = img_lab[:, :, 0] / 100.0  # Normalize L to [0,1]
        img_ab = img_lab[:, :, 1:] / 128.0  # Normalize AB [-1,1]
...
class ViTColorizationModel(LightningModule):
    def __init__(self, model_id="google/vit-base-patch16-224-in21k", dropout_rate=0.5, cache_dir=None):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = F.mse_loss(pred_ab, img_ab)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pred_ab = self(pixel_values)
        loss = F.mse_loss(pred_ab, img_ab)
        self.log("val_loss", loss.cpu(), on_step=True, on_epoch=True, prog_bar=True)
        return loss
...
def main():
    args = parse_args()
    folder_name = f"bs_{args.batch_size}_lr_{args.learning_rate}_epochs_{args.max_epochs}_dropout_{args.dropout_rate}"
    ...
    model = ViTColorizationModel(dropout_rate=args.dropout_rate, cache_dir=CACHE_DIR)
    ...
    trainer = Trainer(
        ...
        callbacks=[checkpoint_callback, LossPlotCallback(), loss_logging_callback],
